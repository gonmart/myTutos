            ...
            Ext.define('CustomApp', {
                ...
                launch: function () {
                    ...
                    if (!Ext.isEmpty(fieldsToCopy.hierarchicalrequirement) && !Ext.isArray(
                            fieldsToCopy.hierarchicalrequirement)) {
                        fieldsToCopy.hierarchicalrequirement = fieldsToCopy.hierarchicalrequirement
                            .split(',');
                    }
                    ...
                    if (app.getSetting('allow_current_project') && app.getSetting(
                            'allow_current_project') != "false") {
                    ...
                    this._getRequiredFields().then({
                        ...
                        success: function (requiredFields) {
                            ...
                            if (Ext.Array.contains(requiredFields.hierarchicalrequirement,
                                    'Release')) {
                ...
                setSummary: function () {
                    ...
                    if (app.list && app.list.length > 0) {
                        summary_array.push({
                            xtype: 'container',
                            html: app.list.length + " Items to be copied."
                        });
                    }
                    ...
                    Ext.create('Rally.ui.dialog.ChooserDialog', {
                        ...
                        listeners: {
                            artifactChosen: function (selectedRecord) {
                                this.down("#item-label").setText(selectedRecord.get(
                                    'Name'));
                ...
                itemSelected: function (root) {
                    ...
                    async.map([config], app.wsapiQuery, function (err, results) {
                        ...
                        async.map([item], app.createList, function (err, results) {
                            ...
                            app.types = _.uniq(_.map(app.list, function (l) {
                                return l.get("_type");
                            }));
                            // load the models for the types
                            async.mapSeries(app.types, app.loadModel, function (err,
                                results) {
                ...
                performCopy: function () {
                    ...
                    async.mapSeries(app.list, app.copyItem, function (err, results) {
                        if (err === null) {
                            app.updateSummary(results.length + " Items copied to " +
                                results[0].get("FormattedID"));
                        } else {
                ...
                copyItem: function (i, callback) {
                    ...
                    var copy = {
                        "Name": app.getSetting('copy_text') + " - " + i.get("Name"),
                        ...
                        "DisplayColor": !_.isNull(i.get("DisplayColor")) && !_.isUndefined(i.get(
                                "DisplayColor")) ?
                    ...
                    async.map([{
                        ...
                        copy: copy,
                        source: i
                    }], app.createItem, function (err, results) {
                ...
                copyTypeSpecificFields: function (copy, item) {
                    ...
                    Ext.Array.each(this.fieldsToCopy[type], function (field) {
                        ...
                        if (!Ext.isEmpty(item_release) && Ext.Array.contains(
                                reference_fields, field)) {
                            copy[field] = {
                                _ref: item_release._ref
                            };
                        } else if (copy[field] === undefined) {
                    ...
                    rec.save({
                        callback: function (result, operation) {
                            if (operation.success === true) {
                                app.copyList[item.source.get("_ref")] = result.get(
                                    "_ref");
                                ...
                                var message =
                                    "<span class='icon-warning'> </span>Create Error when copying " +
                                    item.copy.Name;
                                if (!Ext.isEmpty(operation.error) && !Ext.isEmpty(
                                        operation.error.errors)) {
                                    message += ":<br/>" + operation.error.errors.join(
                                        '<br/>');
                                }
                ...
                createList: function (root, callback) {
                    ...
                    async.map([config], wsapiQuery, function (err, results) {
                        ...
                        if (app.isObject(childRef)) {
                            ...
                            if (app.getSetting('preserve_rank') && app.getSetting(
                                    'preserve_rank') != "false") {
                                async.mapSeries([config], app.readCollection, function (err,
                                    results) {
                                    var children = results[0];
                                    async.mapSeries(children, app.createList,
                                        function (err, results) {
                                            callback(null, results);
                                        });
                                });
                            } else {
                                async.map([config], app.readCollection, function (err,
                                    results) {
                                    var children = results[0];
                                    async.map(children, app.createList, function (
                                        err, results) {
                                        callback(null, results);
                                    });
                                });
                ...
                parentRef: function (obj) {
                    ...
                    if (_.isObject(obj.get("Parent"))) {
                        return {
                            type: "Parent",
                            ref: obj.get("Parent")._ref
                        };
                    }
                    if (_.isObject(obj.get("WorkProduct"))) {
                        return {
                            type: "WorkProduct",
                            ref: obj.get("WorkProduct")._ref
                        };
                    }
                    if (_.isObject(obj.get("PortfolioItem"))) {
                        return {
                            type: "PortfolioItem",
                            ref: obj.get("PortfolioItem")._ref
                        };
                    }
                    return null;
                    ...
                    return [{
                        ...
                        },
                        {
                            ...
                            _shouldShowField: function (field) {
                                ...
                                return attr && !attr.ReadOnly && attr.AttributeType !==
                                    'COLLECTION';
                            },
                            listeners: {
